...
public static class Vorbisfile
{
	...
	[DllImport(nativeLibName, CallingConvention = CallingConvention.Cdecl)]
	public static extern int ov_read(
		IntPtr vf,
		byte[] buffer,
		int length,
		int bigendianp,
    ...
    #region ov_open_callbacks 지원 추가
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    public delegate uint ReadFunc(
        IntPtr ptr, // void*
        uint size,
        uint nmemb,
        IntPtr datasource // void*
    );

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    public delegate int SeekFunc(
        IntPtr datasource, // void*
        int offset,
        int whence
    );
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    public delegate int CloseFunc(
        IntPtr datasource // void*
    );

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    public delegate int TellFunc(
        IntPtr datasource // void*
    );
    ...
    {
        public ReadFunc read_func;
        public SeekFunc seek_func;
        public CloseFunc close_func;
        public TellFunc tell_func;
    }
    ...
    public static int ov_open_callback(IntPtr datasource, out IntPtr vf, IntPtr initial, int ibytes, ov_callbacks callbacks)
    {
        // Do not attempt to understand these numbers at all costs!
        const int win32Size = 720;
        const int unix32Size = 704;
