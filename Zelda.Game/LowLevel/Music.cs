...
namespace Zelda.Game.LowLevel
{
    ...
    class Music
    {
        ...
        public static void FindMusicFile(string musicId, out string fileName, out MusicFormat format)
        {
            ...
            if (MainLoop.ModFiles.DataFileExists(fileNameStart + ".ogg"))
            {
                format = MusicFormat.Ogg;
                fileName = fileNameStart + ".ogg";
            }
            else if (MainLoop.ModFiles.DataFileExists(fileNameStart + ".it"))
            {
        ...
        bool Start()
        {
            ...
            switch (_format)
            {
                case MusicFormat.It:
                    {
                        soundBuffer = MainLoop.ModFiles.DataFileRead(_fileName);
                ...
                case MusicFormat.Ogg:
                    {
                        _oggMem.position = 0;
                        _oggMem.loop = _loop;
                        _oggMem.data = MainLoop.ModFiles.DataFileRead(_fileName);
